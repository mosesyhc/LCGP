...
print("\n=== FITTED PARAMETERS ===")
lLmb, lLmb0, lsigma2s, lnugGPs = modelrun.model.get_param()
print(f"Lengthscales (lLmb) - shape {lLmb.shape}:")
for k in range(lLmb.shape[0]):
    print(f"  Component {k}: {lLmb[k].numpy()}")
...
print(f"Min/Max replications: {np.min(r)}/{np.max(r)}")
