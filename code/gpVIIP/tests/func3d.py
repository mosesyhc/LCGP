...
def forrester2008(x, noisy=True, noiseconst=0.1):
    ...
    if noisy:
        e1 = np.random.normal(0, noiseconst * 0.2 * np.std(y1), x.shape)
        e2 = np.random.normal(0, noiseconst * 2 * np.std(y2), x.shape)
        e3 = np.random.normal(0, noiseconst * 3 * np.std(y3), x.shape)
        y1 += e1
        y2 += e2
        y3 += e3
